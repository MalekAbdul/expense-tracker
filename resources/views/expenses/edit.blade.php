    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 stagger-child">
                <h1 class="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">Edit Expense</h1>
                <p class="text-slate-500 mt-2">Update your spending record (৳)</p>
            </div>
            ...
            <div class="glass-card p-8 stagger-child">
                <form action="{{ route('expenses.update', $expense) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative group">
                                ...
                                <x-text-input id="amount" class="block w-full pl-10 border-red-500/20 focus:border-red-500/50 focus:ring-red-500/50" type="number" step="0.01" name="amount" :value="old('amount', $expense->amount)" required autofocus />
                            </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <x-text-input id="date" class="block w-full" type="date" name="date" :value="old('date', \Carbon\Carbon::parse($expense->date)->format('Y-m-d'))" required />
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="category" name="category" class="block w-full bg-white/5 border-white/10 focus:border-red-500/50 focus:ring-red-500/50 rounded-xl shadow-sm text-slate-200 transition-all" onchange="updateSuggestions(this.value)">
                            @foreach($categories as $category)
                                <option value="{{ $category }}" {{ old('category', $expense->category) === $category ? 'selected' : '' }} class="bg-[#1E293B]">{{ $category }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="description" name="description" class="block w-full bg-white/5 border-white/10 focus:border-red-500/50 focus:ring-red-500/50 rounded-xl shadow-sm text-slate-200 placeholder-slate-500 transition-all min-h-[100px]">{{ old('description', $expense->description) }}</textarea>
